...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Set up Local LLM Conversation."""
    await async_migrate_integration(hass)

    async def render_image(call: ServiceCall) -> ServiceResponse:
        """Render an image using the local LLM."""
    ...
    async def send_prompt(call: ServiceCall) -> ServiceResponse:
        """Send a prompt to Local LLM and return the response."""
        entry_id = call.data["config_entry"]
        entry = hass.config_entries.async_get_entry(entry_id)
...
async def async_update_options(hass: HomeAssistant, entry: LocalLLMConfigEntry) -> None:
    """Update options."""
...
async def async_migrate_entry(hass: HomeAssistant, entry: LocalLLMConfigEntry) -> bool:
    """Migrate entry."""
...
def _add_ai_task_subentry(hass: HomeAssistant, entry: LocalLLMConfigEntry) -> None:
